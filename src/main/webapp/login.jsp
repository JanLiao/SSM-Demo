...
<link rel="stylesheet" href="<%=request.getContextPath()%>/layui/css/login.css" media="all" />
    ...
    if(window !=top){
        ...
        function getMedia(){
            ...
            video.src = "<%=request.getContextPath()%>/media/login/" + videoName[offset]; --%>
        }
    </script>
</head>
...
<script type="text/javascript">
   ...
   function loginIn(){
	   ...
	   if(validateNameAndPasswd()){
		   $.ajax({
			   type:'POST',
			   url:'<%=request.getContextPath()%>/user/loginIn',
			   data:{'userAccount':userAccount,'userPassword':userPassword},
			   dataType:'json',
			   success:function(data){
				   if(data.success) {
				        document.location = "<%=request.getContextPath()%>/main/index";
				     }
			   }
			});
	   }
